...
const parser = port.pipe(new ByteLength({length:2}));    // バイト長で区切る場合
...
parser.on('data', (data) => {
    ...
    const currentAltitude = buf.readUInt8(1);

    console.log(currentStatus + "," + currentAltitude);
});
...
(async () => {
    ...
    await sendCommand(port, [0xA0, 0x0d]);
